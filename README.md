...
## Introduction
Traditionally depth estimation is done using depth sensors such as Intel’s D435i, which are large, heavy, and energy-intensive. Low-energy robots on the other hand have limited energy and load-carrying capacity. Therefore, researchers are now using Deep Neural Networks (DNNs) for depth estimation. Even so, it’s hypothesized that models with better generalization capabilities are more energy-intensive and require data to be ready before training. This is not only limiting for low-energy robots but also limits robot performance in real-world scenarios where data is available sequentially. As a solution, in the LEAN lab, we use smaller DNN models to train miniature-sized robots, online, on the fly. To analyze DNN performance, we use aleatoric (data inherent) and epistemic (model inherent) uncertainty estimation techniques. 

To conduct this research, we collected datasets in interesting and challenging conditions including optical illusion and environments with varying brightness levels. We then run aleatoric and epistemic (ensemble) uncertainty methods from the Uncertainty from Motion algorithm [[1]](#1) on these datasets to qualitatively and quantitatively analyze how both methods perform. This repo contains an end-to-end instruction guide on how to conduct data collection and visualization process as well as running uncertainty estimation methods using the Uncertainty from Motion algorithm. Additionally, the folder contains useful programs such as RGB and depth data visualization (placed adjacently or overlaid one on top of the other). 

## Get Started
Instructions for the data collection process on a Windows OS are listed in the first section. These are steps taken to collect RGB and depth data from the Intel RealSense D435i depth sensor on a Windows machine.

**Note:** Currently, PyRealSense does not have a distribution that’s compatible with the new M1 chip Macbook laptops due to its ARM-based architecture. Therefore, if you have an M1 chip Macbook, I recommend that you conduct at least the data collection process on a Windows machine (Windows dual boot with a Linux OS should be fine). In addition, the Realsense library installation is not supported on a Virtual Machine due to the USB 3.0 translation layer between the native hardware and the virtual machine, see (https://dev.intelrealsense.com/docs/compiling-librealsense-for-linux-ubuntu-guide) for more information.

Instructions on running uncertainty estimation algorithms on collected data are outlined in the second section followed by data visualization instructions in the third section (both using a Linux OS). At the end, there's an additional troubleshooting section, to be used if needed, for running the Uncertainty from Motion algorithm.

## 1. Data Collection
**System Requirements:** Windows 10 and python 3.7 or higher <br>
**Required Packages:** pyrealsense2, opencv-python, numpy 
#### Steps 
If you have python, pip, and git already installed skip steps 1-3. <br>
